            ...
            success: function (data) {
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++){
                        ...
                        newsList.push('<p class="fn-clear"><span class="pleft">'+(list[i].price > 0 ? '<b>'+list[i].price+'</b><span class="symbol">元</span>' : '面议')+'</span> <span class="pright">'+huoniao.transTimes(list[i].pubdate, 3)+'</span></p>');
                        newsList.push('</a>');
                        newsList.push('</li>');
                    }
