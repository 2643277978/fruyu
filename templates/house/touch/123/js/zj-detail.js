			    ...
			    success: function(data){
					if(data.state == 100){
				        ...
				        for(var i = 0; i < list.length; i++){
				        	...
				        	var price = list[i].price>0 ? '<strong>'+list[i].price+'</strong><span>元/月</span>' : '<strong>面议</strong>';
						...
						if(list.length > 0){
							if(action=="team"){
								for(var i = 0; i < list.length; i++){
								    ...
								    html.push('<a href="'+list[i].url+'">');
								    html.push('<div class="house-item">');
								    html.push('<div class="jy_img l"><img src="'+huoniao.changeFileSize(list[i].photo, "small")+'"></div>');
								    ...
								    html.push(' <dd class="item-type-1 jy-item-type-1 fn-clear"><em>'+list[i].address[0]+list[i].address[1]+list[i].address[2]+'</em><button class="r">进入店铺</button></dd>');
								    html.push('</dl>');
								    html.push('</div>');
								    html.push('<div class="clear"></div>');
								    html.push('</a>');
								...
								for(var i = 0; i < list.length; i++){
	                            	...
	                            	var price = '';
	                            	if(list[i].price>0){
	                            		price = '<span class="price r">'+list[i].price+'万元</span>';
		                            }else{
								    ...
								    html.push('<dd class="item-type-1 fn-clear"><em class="l">'+list[i].community+'</em><em class="r">均价 '+list[i].unitprice+'元/㎡</em></dd>');
								    html.push('</dl>');
								    html.push('</div>');
								    html.push('<div class="clear"></div>');
								    html.push('</a>');
								...
								$(".esfList .house-list").append(html.join(''));
								 ...
								 for(var i = 0; i < list.length; i++){
	                            	...
	                            	var price = '';
	                            	if(list[i].price>0){
	                            		price = '<span class="price r">'+list[i].price+'元/月</span>';
		                            }else{
		                            	price = '<span class="price r">面议</span>';
		                            }
								    html.push('<i class="house_disk"></i><img src="'+pic+'">'+ivplay+ivr);
