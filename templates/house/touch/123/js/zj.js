$(function() {
	...
	$('#store-list').css('min-height', $(window).height() - $('.footer').height());
  ...
  setTimeout(function(){detailList.removeLocalStorage();}, 800);
	...
	$('#store-list').delegate('.house-box', 'click', function(){
		...
		detailList.insertHtmlStr(dataInfo, $("#store-list").html(), {lastIndex: atpage});
	...
	function getList(tr){
		...
		$.ajax({
            url: masterDomain+"/include/ajax.php?service=house&action=zjComList",
            data: data.join("&"),
            type: "GET",
            dataType: "json",
			success: function(data){
					...
					if(data.state == 100){
						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								...
								html.push('<div class="house-img l"><img src="'+litpic+'"></div>');
								html.push('<dl class="l">');
								var set_top = '';
								if(list[i].isbid==1){
									set_top = '<i class="set_top"></i>';
								}
								html.push('<dt class="fn-clear">'+set_top+'<em class="sp_title">'+list[i].title+'</em><button>进入店铺</button></dt>');
								html.push('<dd class="item-area xzl-item-area"><span>出租<em class="s">'+list[i].countSale+'</em><em class="x">|</em>出售<em class="s">'+list[i].countZu+'</em><em class="x">|</em>团队<em class="s">'+list[i].countTeam+'</em></span></dd>');
								html.push('<dd class="item-type-1 xzl-item-type-1">'+list[i].addrName[list[i].addrName.length - 2] + '&nbsp;' + list[i].addrName[list[i].addrName.length - 1] + '&nbsp;' + list[i].address+'</dd>');
								html.push('</dl>');
								html.push('</div>');
