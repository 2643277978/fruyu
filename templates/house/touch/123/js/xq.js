					...
					if(data.state == 100){
						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								...
								html.push('<dd class="item-area xq-item-area"><em>在售<span>'+list[i].saleCount+'</span>套|在租<span>'+list[i].zuCount+'</span>套|' + huoniao.transTimes(list[i].opendate, 2).split('-')[0] + '年建成</em></dd>');
