  ...
  $('.h-menu').on('click', function() {
    ...
    $('.mask').on('touchstart',function(){
      ...
      // data = data + (data != "" ? "&" : "") + moreFilter.join("&");
			...
			$.ajax({
				...
				"success": function(data){
					...
					if(data && data.state == 100){
						...
						for(var i = 0; i < list.length; i++){
							saleData[i] = [];
							saleData[i]['sale_id'] = list[i].id;
							saleData[i]['longitude'] = list[i].longitude;
							saleData[i]['latitude'] = list[i].latitude;
							saleData[i]['resblock_name'] = list[i].title;
							saleData[i]['loupan_addr'] = list[i].address;
							saleData[i]['ptype'] = list[i].ptype;
							saleData[i]['average_price'] = list[i].price;
							saleData[i]['cover_pic'] = list[i].litpic;
							saleData[i]['house_type'] = list[i].protype;
							saleData[i]['url'] = list[i].url;
						}

					}

					g_conf.loupanData = saleData;
					init.doNext(type);
		...
		,doNext: function(type){
			if(g_conf.districtData && g_conf.saleData){
		...
		,createBubble: function(data, temp, resize,more){
			...
			$.each(data,	function(e, o) {
				...
				map.addOverlay(bubbleLabel);
					// 统计可视区域内的数量
					data = resize == 1 ? init.getVisarea(g_conf.saleData) : data;
	...
	var bubbleMapSize = {
		...
		,bubbleTemplate = {
			...
			//区域
			1 : '<div class="bubble bubble-1" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${sale_id}"><p class="name" title="${name}区">${name}区</p><p class="num"></p><p><span class="count">${house_count}</span>套</p></div>',

		//楼盘、价格及类型
		2 : '<div class="bubble bubble-2 bubble-3" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${zu_id}"><div class="bubble-wrap"><div class="bubble-inner"><a href="${href}" target="_blank">${moreTpl}</a></div><i class="arrow"><i class="arrow-i"></i></i></div><p class="cycle"></p></div>',
		//楼盘价格
