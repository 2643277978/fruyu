...
//图片放大
    ...
    //大图切换
    $(".sp_slide").slide({titCell: ".plist li",mainCell: ".album",effect: "fold",autoPlay: true,delayTime: 500,switchLoad: "_src",pageStateCell:".pageState",startFun: function(i, p) {if (i == 0) {$(".sprev").click()} else if (i % 5 == 0) {$(".snext").click()}}});

    //小图左滚动切换
    $(".sp_slide .thumb").slide({mainCell: "ul",delayTime: 300,vis: 5,scroll: 5,effect: "left",autoPage: true,prevCell: ".sprev",nextCell: ".snext",pnLoop: false});
    ...
    $(".btnJb").bind("click", function(){
        ...
        $.dialog({
            fixed: false,
            title: "房源举报",
            content: 'url:'+domainUrl+'/complain-house-sp-'+id+'.html',
            width: 460,
            height: 300
        });
    });
    ...
    $('.btnSc').click(function(){
        ...
        $.post("/include/ajax.php?service=member&action=collect&module=house&temp=sp_detail&type="+type+"&id="+id);
    });

    //增加浏览历史
    var house_sp_history = $.cookie(cookiePre+'house_sp_history');
    if (house_sp_history == null) house_sp_history = "";
    if (house_sp_history.indexOf(pageData.id) == -1) {
        if (house_sp_history.length > 0) {
            house_sp_history += ':'+pageData.id;
        } else {
            house_sp_history += pageData.id;
        }
        if (house_sp_history.length > 128) {
            var pos = house_sp_history.indexOf(':');
            house_sp_history = house_sp_history.substr(pos + 1);
        }
        $.cookie(cookiePre+'house_sp_history', house_sp_history, {expires: 365, domain: masterDomain.replace("http://", "").replace("https://", ""), path: '/'});
    }
