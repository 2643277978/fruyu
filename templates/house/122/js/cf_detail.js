$(function() {
  ...
  //大图切换
  $(".cf_slide").slide({titCell: ".plist li",mainCell: ".album",effect: "fold",autoPlay: true,delayTime: 500,switchLoad: "_src",pageStateCell:".pageState",startFun: function(i, p) {if (i == 0) {$(".sprev").click()} else if (i % 5 == 0) {$(".snext").click()}}});

  //小图左滚动切换
  $(".cf_slide .thumb").slide({mainCell: "ul",delayTime: 300,vis: 5,scroll: 5,effect: "left",autoPage: true,prevCell: ".sprev",nextCell: ".snext",pnLoop: false});
  ...
  $("body").delegate("#tj", "click", function(){
      ...
      $('.txt_other dl').each(function(i){
        if(i == 5) return;
        var s = $(this).children('dd').text();
        s = $.trim(s).replace(/ /g, '');
        if(s != ''){
          title += ' · ' + s;
        }
...
window._bd_share_config={"common":{"bdSnsKey":{},"bdText":"","bdMini":"1","bdMiniList":["tsina","tqq","qzone","weixin","sqq","renren"],"bdSize":"16"},"share":{"bdSize":0}};with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src=shareApiUrl];
  ...
  $(".btnJb").bind("click", function(){
    ...
    $.dialog({
      fixed: false,
      title: "房源举报",
      content: 'url:'+domainUrl+'/complain-house-cf-'+pageData.id+'.html',
      width: 460,
      height: 300
    });
  });
  ...
  collect('add', 1, function(data){
    if(data && data.state == 100){
      if(data.info == 'btnYsc'){
        $('.btnSc').html("<i></i>已收藏").addClass('btnYsc');
      }
    }
  }); 

  //增加浏览历史
  var house_cf_history = $.cookie(cookiePre+'house_cf_history');
  if (house_cf_history == null) house_cf_history = "";
  if (house_cf_history.indexOf(pageData.id) == -1) {
    if (house_cf_history.length > 0) {
      house_cf_history += ':'+pageData.id;
    } else {
      house_cf_history += pageData.id;
    }
    if (house_cf_history.length > 128) {
      var pos = house_cf_history.indexOf(':');
      house_cf_history = house_cf_history.substr(pos + 1);
    }
    $.cookie(cookiePre+'house_cf_history', house_cf_history, {expires: 365, domain: masterDomain.replace("http://", "").replace("https://", ""), path: '/'});
  }
