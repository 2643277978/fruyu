...
sjkf( $(".mobile_kf_mid #sj"));
  ...
  function countDown(obj,time){
   ...
   //大图切换
  $(".zu_slide").slide({titCell: ".plist li",mainCell: ".album",effect: "fold",autoPlay: true,delayTime: 500,switchLoad: "_src",pageStateCell:".pageState",startFun: function(i, p) {if (i == 0) {$(".sprev").click()} else if (i % 5 == 0) {$(".snext").click()}}});

  //小图左滚动切换
  $(".zu_slide .thumb").slide({mainCell: "ul",delayTime: 300,vis: 5,scroll: 5,effect: "left",autoPage: true,prevCell: ".sprev",nextCell: ".snext",pnLoop: false});
...
window._bd_share_config={"common":{"bdSnsKey":{},"bdText":"","bdMini":"1","bdMiniList":["tsina","tqq","qzone","weixin","sqq","renren"],"bdSize":"16"},"share":{"bdSize":0}};with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src=shareApiUrl];
  ...
  $('.btnSc').click(function(){
    ...
    $.post("/include/ajax.php?service=member&action=collect&module=house&temp=zu_detail&type="+type+"&id="+pageData.id);
  });

  //增加浏览历史
  var house_zu_history = $.cookie(cookiePre+'house_zu_history');
  if (house_zu_history == null) house_zu_history = "";
  if (house_zu_history.indexOf(pageData.id) == -1) {
    if (house_zu_history.length > 0) {
      house_zu_history += ':'+pageData.id;
    } else {
      house_zu_history += pageData.id;
    }
