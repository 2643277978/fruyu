$(function () {
    ...
    function checkFilter(){
      ...
      $('.filterlist dl').each(function(g){
        ...
        // 价格
            ...
            if( (pagetype == 'cw_zu' && g == 2) || (pagetype == 'cw_sale' && g == 1) || pagetype == 'cw_zr' ){
                return;
            }
            curr = box.find('.curr');
            index = curr.index();
    ...
    function getList() {
        ...
        var url = masterDomain + "/include/ajax.php?service=house&action=cwList";
                ...
                if (data.state == 100) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<a href="' + d.url + '" target="_blank"><h2>' + d.title + '</h2>' + (d.isbid == "1" ?
                            '<i class="mtop"></i>' : '') + '</a>');
                        html.push('<span class="lpprice">' + (d.price > 0 ? '<b>' + d.price + '</b>' + (d.type == '1' ? '万元' : '元/月') : '<b>价格面议</b>') +
                            '</span>');
                        html.push('</div>');
                        ...
                        html.push('<em>|</em>');
                        if (d.proprice > 0) {
                            html.push('<span>物业费：' + d.proprice + '</span>');
                            html.push('<em>|</em>');
                        }
                        if (d.mintime) {
                            html.push('<span>起租：' + d.mintime + '</span>');
                            html.push('<em>|</em>');
                        }
                        ...
                        if(d.type == 0){
                            if(d.price > 0){
                                html.push('<div class="sp_r fn-right">' + Math.ceil(d.price / d.area) + ' 元/月·平方</div>');
                            }
                        }else if (d.type == "1") {
                            if(d.price > 0){
                                html.push('<div class="sp_r fn-right">' + Math.ceil(d.price / d.area) + ' 万/月</div>');
                            }
                        }else if (d.type == "2") {
                    ...
                    $(".totalCount b").html(0);
                    $(".lplist ul").html('<div class="empty">抱歉！ 未找到相关信息</div>');
                }
            },
            error: function(){
