$(function(){
	...
	var map, filterData, list = $(".list"), init = {
		...
		,closeHx: function(){
			...
			isClickHx = false;
			$(".map-os").show();
			loupanChooseData.longitude && loupanChooseData.latitude ? (map.centerAndZoom(new BMap.Point(loupanChooseData.longitude, loupanChooseData.latitude), 14)) : map.setZoom(14);
			init.updateOverlays();
		...
		,loupanNearbyInfo: function(hxData){
			...
			init.createBubble(loupanChooseDataArr, bubbleTemplate[4], 4);
			map.centerAndZoom(new BMap.Point(loupanChooseData.longitude, loupanChooseData.latitude), 15);

			//周边信息
			var circle = new BMap.Circle(new BMap.Point(Math.max(loupanChooseData.longitude, loupanChooseData.latitude), Math.min(loupanChooseData.longitude, loupanChooseData.latitude)), 1000, {
				fillColor: "#4285f4",
				strokeColor: "#4285f4",
				strokeWeight: 1,
				fillOpacity: .1,
			...
			var localSearch = new BMap.LocalSearch(map, {
				...
				onSearchComplete: function(a) {
					if (localSearch.getStatus() == BMAP_STATUS_SUCCESS) {
						...
						$.each(a, function(e, t) {
							i[e] = [];
							for (var a = 0; a < t.getCurrentNumPois(); a++) i[e].push({
								tle: t.getPoi(a).title,
								addr: t.getPoi(a).address
							})
						});

						//填充周边信息统计数据
						var t = $(".bubble-4");
						t.length && t.find(".around-li").each(function(t, a) {
							$(this).find("span").html(i[t].length)
						})
