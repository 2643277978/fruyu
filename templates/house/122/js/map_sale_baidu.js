$(function(){
	...
	var map, filterData, list = $(".list"), ids = 0, init = {
					...
					"success": function(data){
						...
						if(data && data.state == 100){
							...
							for(var i = 0; i < _list.length; i++){
								...
								saleDate[i]['price_total'] = _list[i].price;
		...
		,doNext: function(type){
			if(g_conf.districtData && g_conf.saleDate){
		...
		,updateOverlays: function(type){
			...
			if(zoom - g_conf.minZoom <= 1){
				...
				if(zoom - g_conf.minZoom ==1){
					...
					data = init.getVisarea(g_conf.saleDate);
					console.log(data);
		...
		,createBubble: function(data, temp, resize,more){
			...
			//区域集合时统计数据为楼盘的数量
			data = resize == 1 ? init.getVisarea(g_conf.saleDate) : data;
