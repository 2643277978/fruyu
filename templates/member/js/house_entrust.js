	...
	objId.delegate(".bj", "click", function(){
		...
		if(id){
			...
			$.dialog.confirm(title, function(){
				...
				$.ajax({
					url: masterDomain+"/include/ajax.php?service=house&action=operaEntrust&iszj=1&state="+newstate+"&id="+id,
					type: "GET",
					dataType: "jsonp",
	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][543], function(){  //你确定要删除这条信息吗？
				...
				$.ajax({
					url: masterDomain+"/include/ajax.php?service=house&action=operaEntrust&iszj=1&state=-1&id="+id,
					type: "GET",
					dataType: "jsonp",
					success: function (data) {
						if(data && data.state == 100){
...
function getList(is){
	...
	$.ajax({
		url: masterDomain+"/include/ajax.php?service=house&action=myEntrust&iszj=1&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
				...
				if(data.state == 101){
					...
					if(list.length > 0){
						for(var i = 0; i < list.length; i++){
							...
							html.push('<table data-id="'+id+'" class="oh"> <colgroup> <col style="width:5%;"> <col style="width:40%;"> <col style="width:15%;"> <col style="width:15%;"> <col style="width:15%;"> <col style="width:25%;"> </colgroup> <thead> <th></th> <th class="tl">'+langData['siteConfig'][31][115]+'</th> <th>'+langData['siteConfig'][31][105]+'</th> <th>'+langData['siteConfig'][31][106]+'</th> <th>'+langData['siteConfig'][6][11]+'</th> </thead> <tbody>');
							//委托客户---委托类型---委托时间---操作
							html.push('<tr>');
					  	html.push('	<td></td>');
					  	...
					  	if(type == 2){
						  	html.push('	<p class="title">'+langData['siteConfig'][31][109] +'：'+(price == 0 ?  langData['siteConfig'][31][108] : price)+langData['siteConfig'][31][111] +'</p>');
						  	html.push('	<p class="title">'+langData['siteConfig'][19][120] +'：'+(transfer == 0 ?  langData['siteConfig'][31][108] : transfer + '元')+'</p>');
						  }else{
