$(function () {
    ...
    $(".car-list").delegate(".del", "click", function(){
        ...
        if(id){
            ...
            if(result){
                $.ajax({
                    url: masterDomain+"/include/ajax.php?service=pension&action=operAlbums&oper=del&id="+id,
                    type: "GET",
                    dataType: "jsonp",
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain+"/include/ajax.php?service=pension&action=albumsList&u=1&page="+atpage+"&pageSize="+pageSize,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<div class="car-img fn-left"><a href="'+url+'"><img src="'+huoniao.changeFileSize(list[i].litpic, "small")+'" alt=""></a></div>');
                        html.push('<dl><a href="'+url+'">');
                        html.push('<dt class="name">'+list[i].title+'</dt>');
                        html.push('</a></dl>');
                        html.push('</div>');
