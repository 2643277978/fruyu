							...
							success: function(res) {
								...
								$.ajax({
									...
									success: function (data) {
										if (data.state == 100) {
											var fid = data.fid, url = data.url, turl = data.turl;
											uploadbtn.after('<li id="' + fid + '" class="thumbnail"><img src="'+turl+'" data-val="'+fid+'"><div class="file-panel"><span class="cancel"></span></div></li>');
										}else {
											alert(data.info);
										}
									},
  ...
  function createUploader(id) {
    ...
    //删除已上传视频
      ...
      if (id == "1") {
        ...
        var $li = $('<li id="' + file.id + '" class="thumbnail"> <video class="video-js" src="" data-val=""><source src="" type="video/mp4"></video></li>')
        $btns = $('<div class="file-panel"><span class="cancel"></span></div>').appendTo($li);
        $img = $li.find('video');
      }
  	...
  	uploader.on('uploadSuccess', function(file, response){
        ...
        if (id == "2") {
          ...
          $li.html('<video class="video-js" id="video_'+file.id+'" src="/include/attachment.php?f='+response.url+'" data-val="'+response.url+'"><source src="/include/attachment.php?f='+response.url+'" type="video/mp4" data-val="'+response.url+'"></video>');
