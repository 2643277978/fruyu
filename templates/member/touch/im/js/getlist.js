	          		...
	          		for(var i=0; i<datalist.length; i++){
		            	...
		            	if(type=='commt'){
							...
							if(detail.litpic){
								...
								list.push('<div class="left_img"><img src="'+detail.imgGroup[0]+'" /></div>');
								list.push('<div class="right_box"><h2>'+((datalist[i].type=='0')?"我":detail.member.nickname)+'：</h2><p class="con">'+(detail.title)+'</p></div>');
							}else{
								list.push('<div class="right_box" style=" margin-left:.2rem;"><h2>'+((datalist[i].type=='0')?"我":detail.member.nickname)+'：</h2><p class="con">'+detail.title+'</p></div>');
							}
			          }
						if(className=="con_box"){
...
//	          		var area = datalist[i].addrName?datalist[i].addrName.split('>'):'暂无';
					...
					list.push('<a data-id="'+datalist[i].userid+'" href="'+userDomain+'chat-'+datalist[i].userid+'.html" class="im-chat_width">私聊</a>');
					if(!datalist[i].isfriend){
						list.push('<a href="'+userDomain+'f_test-'+datalist[i].userid+'.html" class="im-add_btn">加好友</a>');
					}else{
