$(function () {
  ...
  var lng = "", lat = "";
    ...
    $('#submit').click(function (e) {
        ...
        if(!elderlyname){
          r = false;
          showMsg('请输入老人姓名'); //请输入联系人
          return;
        }else if(!people){
          r = false;
          showMsg('请输入联系人');  //请输入联系人
          return;
        }else if(!tel){
          r = false;
          showMsg('请输入联系方式');  //请输入联系方式
          return;
        }else if(!relationship){
          r = false;
          showMsg('请输入亲属关系');  //请输入亲属关系
          return;
        }
        ...
        $.ajax({
            ...
            success: function(data){
                if(data && data.state == 100){
                    ...
                    $("#submit").removeClass("disabled").html('立即发布');		//立即发布
                    showMsg(tip);
                }else{
                  showMsg(data.info);
                  $("#submit").removeClass("disabled").html('立即发布');		//立即发布
