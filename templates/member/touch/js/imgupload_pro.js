$(function(){
	...
	//上传凭证
	var $list = $('#fileList_pro'),
		uploadbtn = $('#uploadbtn_pro'),
	...
	if (ua.match(/MicroMessenger/i) == "micromessenger") {
		...
		wx.ready(function() {
			$('#filePicker_pro').bind('click', function(){
...
//			$('#uploadbtn').append('<li>1111<li>')
		...
		uploader.on('uploadComplete', function(file){
			$('#'+file.id+'_pro').find('.progress').remove();
		});
...
//		alert('111')
	...
	function addFile(file) {
		var $li= $('<li id="' + file.id + '_pro" class="thumbnail imgshow_box"><div class="img_show"><img></div></li>');
