		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanOrderList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#list").delegate(".refund", "click", function(){
		...
		if(id != ""){
			$.dialog.confirm('此操作不可恢复，您确定要退款吗？', function(){
				huoniao.showTip("loading", "正在操作，请稍候...");
				huoniao.operaJson("tuanOrderList.php?dopost=refund", "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() { 
							getList();
...
function getList(){
	...
	huoniao.operaJson("tuanOrderList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < tuanOrderList.length; i++){
				...
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+tuanOrderList[i].userid+'" class="userinfo">'+tuanOrderList[i].username+'</a></td>');
				...
				list.push('  <td class="row7 left">&yen;'+tuanOrderList[i].orderprice+'</td>');
				...
				if(tuanOrderList[i].orderstate == "1" || tuanOrderList[i].orderstate == "2" || tuanOrderList[i].orderstate == "4" || tuanOrderList[i].orderstate == "6"){
					btn = '<a href="javascript:;" data-id="'+tuanOrderList[i].id+'" class="refund" title="退款">退款</a>';
				}
				list.push('  <td class="row10 left"><a data-id="'+tuanOrderList[i].id+'" data-title="'+tuanOrderList[i].ordernum+'" href="tuanOrderEdit.php?dopost=edit&id='+tuanOrderList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a>'+btn+'</td>');
			}
