$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("datingSchool.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("datingSchool.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingSchoolAdd", "dating", "新增信息", "dating/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingSchoolEdit"+id, "dating", title, "dating/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除选中的信息吗？<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
	...
	$("#list").delegate(".del", "click", function(){
		$.dialog.confirm('确定后将删除此条信息吗？<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("datingSchool.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, datingSchool = val.datingSchool;
		...
		if(val.state == "100"){
			...
			for(i; i < datingSchool.length; i++){
				list.push('<tr data-id="'+datingSchool[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><img src="'+cfg_attachment+datingSchool[i].litpic+'&type=small" width="100" class="litpic" /><a href="'+datingSchool[i].url+'" target="_blank">'+datingSchool[i].title+'</a></td>');
				list.push('  <td class="row15 left">'+datingSchool[i].type+'</td>');
				var state = "";
				switch (datingSchool[i].arcrank) {
					case "0":
				...
				list.push('  <td class="row20 left">'+datingSchool[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+datingSchool[i].id+'" data-title="'+datingSchool[i].title+'" href="datingSchool.php?dopost=edit&id='+datingSchool[i].id+'" title="修改" class="edit">修改</a>');
