	...
	huoniao.operaJson("memberList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < memberList.length; i++){
				...
				list.push('  <td class="row5 left">'+mtype+'<br />ID:'+memberList[i].id+'</td>');
				list.push('  <td class="row14 left"><img onerror="javascript:this.src=\'/static/images/default_user.jpg\';" src="'+photo+'" class="litpic" style="width:60px; height:60px;" /><span>'+memberList[i].username+'<br /><small>'+memberList[i].addrname+'</small></span></td>');
				var company = "";
				var certify = "";
				var level = '<br /><small><span class="label"style="margin-right:3px;" title="普通会员">普</span>';
				if(memberList[i].level){
					level = '<br /><small><span class="label label-info"style="margin-right:3px;" title="'+memberList[i].level+"\r过期时间："+huoniao.transTimes(memberList[i].expired, 2)+'">'+memberList[i].level.split("")[0]+'</span>';
				}
				...
				if(memberList[i].mtype == 2){
					...
					company = '<span class="label'+certify+'"style="margin-right:3px;" title="营业执照认证">验</span>'+memberList[i].company+'</small>';
				}else{
					...
					company = '<span class="label'+certify+'"style="margin-right:3px;" title="身份证实名认证">验</span>'+memberList[i].realname+'</small>';
				}


				list.push('  <td class="row13 left">'+memberList[i].nickname+'【'+memberList[i].sex+'】'+level+company+'</td>');
				...
				list.push('  <td class="row6 left">'+memberList[i].money+'<br />'+memberList[i].promotion+'</td>');
