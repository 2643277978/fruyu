	...
	huoniao.operaJson("loupanList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < loupanList.length; i++){
				...
				list.push('  <td class="row25 left">'+img+'<span><a href="'+loupanList[i].url+'" target="_blank">'+loupanList[i].title+'</a>'+append+'<br /><small style="display: inline; color: #666;">'+loupanList[i].date+'</small></span></td>');
