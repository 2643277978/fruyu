$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseSp.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseSp.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("houseSpEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseSp.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseSp = val.houseSp;
		...
		if(val.state == "100"){
			...
			for(i; i < houseSp.length; i++){
				list.push('<tr data-id="'+houseSp[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+houseSp[i].litpic+'&type=small" class="litpic" />';
				var append = houseSp[i].type == 0 ? "(租)" : "(转)";
				list.push('  <td class="row30 left">'+img+'<span><a href="'+houseSp[i].url+'" target="_blank">'+houseSp[i].title+'</a><span class="append">'+append+'</span></span></td>');
				list.push('  <td class="row5">'+houseSp[i].weight+'</td>');
                list.push('  <td class="row9">'+houseSp[i].addrname+'</td>');
				var type = houseSp[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseSp[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseSp[i].userid+'" class="userinfo">'+houseSp[i].username+'</a>';
				}else{
					username = houseSp[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+houseSp[i].contact+'</td>');
				var state = "";
				switch (houseSp[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+houseSp[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+houseSp[i].id+'" data-title="'+houseSp[i].title+'" href="houseSpAdd.php?dopost=edit&id='+houseSp[i].id+'" title="修改" class="edit">修改</a>');
