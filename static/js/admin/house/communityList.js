$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("communityList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("communityList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("apartmentcommunity"+id, "house", title + "户型", "house/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".album", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("housealbumcommunity"+id, "house", title + "相册", "house/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('删除信息将会删除信息下面的所有二手房、租房、户型、相册、问答。<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("communityList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < communityList.length; i++){
				...
				var img = '<img src="'+cfg_attachment+communityList[i].litpic+'&type=small" class="litpic" />';
				list.push('  <td class="row35 left">'+img+'<span><a href="'+communityList[i].url+'" target="_blank">'+communityList[i].title+'</a><br /><small style="display: inline; color: #666;">'+communityList[i].date+'</small></span></td>');
				list.push('  <td class="row20"><a href="javascript:;" data-id="'+communityList[i].addrid+'" class="addr">'+communityList[i].addr+'</a></td>');
				// list.push('  <td class="row10">'+communityList[i].username+'</td>');
				list.push('  <td class="row5">'+communityList[i].weight+'</td>');
				list.push('  <td class="row7">&yen;'+communityList[i].price+'</td>');
				var state = "";
				switch (communityList[i].state) {
					case "0":
				...
				list.push('<a data-id="'+communityList[i].id+'" data-title="'+communityList[i].title+'" href="communityAdd.php?dopost=edit&id='+communityList[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+communityList[i].id+'" data-title="'+communityList[i].title+'" href="apartment.php?action=community&id='+communityList[i].id+'" title="户型" class="link apart">户型</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+communityList[i].id+'" data-title="'+communityList[i].title+'" href="housealbum.php?action=community&id='+communityList[i].id+'" title="相册" class="link album">相册</a>&nbsp;|&nbsp;');
