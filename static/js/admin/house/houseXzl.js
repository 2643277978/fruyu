$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseXzl.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseXzl.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("houseXzlEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseXzl.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseXzl = val.houseXzl;
		...
		if(val.state == "100"){
			...
			for(i; i < houseXzl.length; i++){
				list.push('<tr data-id="'+houseXzl[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+houseXzl[i].litpic+'&type=small" class="litpic" />';
				var append = houseXzl[i].type == 0 ? "(租)" : "(售)";
				list.push('  <td class="row25 left">'+img+'<span><a href="'+houseXzl[i].url+'" target="_blank">'+houseXzl[i].title+'</a><span class="append">'+append+'</span></span></td>');
                list.push('  <td class="row5">'+houseXzl[i].weight+'</td>');
				list.push('  <td class="row9 left">'+houseXzl[i].addrname+'</td>');
				list.push('  <td class="row10 left">'+houseXzl[i].loupan+'</td>');

				var type = houseXzl[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseXzl[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseXzl[i].userid+'" class="userinfo">'+houseXzl[i].username+'</a>';
				}else{
					username = houseXzl[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+houseXzl[i].contact+'</td>');
				var state = "";
				switch (houseXzl[i].state) {
					case "0":
				...
				list.push('  <td class="row8 state">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row12">'+houseXzl[i].pubdate+'</td>');
				list.push('  <td class="row8">');
				list.push('<a data-id="'+houseXzl[i].id+'" data-title="'+houseXzl[i].title+'" href="houseXzlAdd.php?dopost=edit&id='+houseXzl[i].id+'" title="修改" class="edit">修改</a>');
