$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseSale.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseSale.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("houseSaleEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseSale.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseSale = val.houseSale;
		...
		if(val.state == "100"){
			...
			for(i; i < houseSale.length; i++){
				list.push('<tr data-id="'+houseSale[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+houseSale[i].litpic+'&type=small" class="litpic" />';
				list.push('  <td class="row25 left">'+img+'<span><a href="'+houseSale[i].url+'" target="_blank">'+houseSale[i].title+'</a></span></td>');
				list.push('  <td class="row10">'+houseSale[i].weight+'</td>');

				var community = "";
				if(houseSale[i].communityid != 0){
					community = '<a href="'+houseSale[i].communityUrl+'" target="_blank">'+houseSale[i].community+'</a>';
				}else{
					community = houseSale[i].community;
				}
				list.push('  <td class="row12 left">'+community+'<br><small>'+houseSale[i].addrname+'</small></td>');

				var type = houseSale[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseSale[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseSale[i].userid+'" class="userinfo">'+houseSale[i].username+'</a>';
				}else{
					username = houseSale[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+houseSale[i].contact+'</td>');
				var state = "";
				switch (houseSale[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+houseSale[i].pubdate+'</td>');
				list.push('  <td class="row8">');
				list.push('<a data-id="'+houseSale[i].id+'" data-title="'+houseSale[i].title+'" href="houseSaleAdd.php?dopost=edit&id='+houseSale[i].id+'" title="修改" class="edit">修改</a>');
