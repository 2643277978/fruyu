$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseNewsList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		var href = $(this).attr("href"), type = "house";
		try {
			event.preventDefault();
			parent.addPage("houseNewsListphpdopostAdd", type, "新增房产资讯", "house/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("houseNewsEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseNewsList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseNewsList = val.houseNewsList;
		if(val.state == "100"){
			huoniao.hideTip();

			obj.attr("data-totalpage", val.pageInfo.totalPage);

			for(i; i < houseNewsList.length; i++){
				list.push('<tr data-id="'+houseNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><a href="'+houseNewsList[i].url+'" target="_blank">'+houseNewsList[i].title+'</a></td>');
				list.push('  <td class="row5">'+houseNewsList[i].cityname+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+houseNewsList[i].typeid+'" class="type">'+houseNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+houseNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+houseNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+houseNewsList[i].id+'" data-title="'+houseNewsList[i].title+'" href="houseNewsList.php?dopost=Edit&id='+houseNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
