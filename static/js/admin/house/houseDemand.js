$(function(){
		...
		init = {
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseDemand.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseDemand.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
							//huoniao.showTip("success", data.info, "auto");
							$("#selectBtn a:eq(1)").click();
							setTimeout(function() {
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#sAction").html($("#actionBtn").val());
		$("#sType").html($("#typeBtn").val());
		$("#sAddr").html($("#addrBtn").val());
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("houseDemandAdd", "house", "新增求租/求购", "house/"+href);
		} catch(e) {}
	});

	//二级菜单点击事件
	$("#actionBtn a").bind("click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#actionBtn").attr("data-id", id);
		$("#actionBtn button").html(title+'<span class="caret"></span>');
	});
	...
	$(".dropdown-toggle").bind("click", function(){
		if($(this).parent().attr("id") != "addrBtn"){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("houseDemandEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var data = [];
		...
		data.push("sAction="+sAction);
		data.push("sType="+sType);
		// data.push("sAddr="+sAddr);
		data.push("cityid="+sAddr);
	...
	huoniao.operaJson("houseDemand.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseDemandList = val.houseDemandList;
		...
		if(val.state == "100"){
			...
			for(i; i < houseDemandList.length; i++){
				list.push('<tr data-id="'+houseDemandList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left">'+houseDemandList[i].title+'</td>');
				list.push('  <td class="row5">'+houseDemandList[i].weight+'</td>');
				var action = "";
				switch (houseDemandList[i].action) {
					case "1":
						action = "新房";
						break;
					case "2":
				...
				list.push('  <td class="row7">'+action+'</td>');
				var type = "求租";
				if(houseDemandList[i].type == 1){
					type = "求购";
				}
				list.push('  <td class="row7">'+type+'</td>');
				list.push('  <td class="row9 left">'+houseDemandList[i].addr+'</td>');
				list.push('  <td class="row10 left">'+houseDemandList[i].person+'</td>');
				list.push('  <td class="row10 left">'+houseDemandList[i].contact+'</td>');
				list.push('  <td class="row10 left">'+houseDemandList[i].date+'</td>');
				var state = "";
				switch (houseDemandList[i].state) {
					case "0":
				...
				list.push('  <td class="row9">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row10"><a data-id="'+houseDemandList[i].id+'" data-title="'+houseDemandList[i].title+'" href="houseDemand.php?dopost=Edit&id='+houseDemandList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
