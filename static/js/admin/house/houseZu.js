$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseZu.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseZu.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("houseZuEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseZu.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseZu = val.houseZu;
		...
		if(val.state == "100"){
			...
			for(i; i < houseZu.length; i++){
				list.push('<tr data-id="'+houseZu[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+houseZu[i].litpic+'&type=small" class="litpic" />';
				list.push('  <td class="row25 left">'+img+'<span><a href="'+houseZu[i].url+'" target="_blank">'+houseZu[i].title+'</a></span></td>');
				list.push('  <td class="row5">'+houseZu[i].weight+'</td>');

				var community = "";
				if(houseZu[i].communityid != 0){
					community = '<a href="'+houseZu[i].communityUrl+'" target="_blank">'+houseZu[i].community+'</a>';
				}else{
					community = houseZu[i].community;
				}
				list.push('  <td class="row12 left">'+community+'<br><small>'+houseZu[i].addrname+'</small></td>');

				var type = houseZu[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseZu[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseZu[i].userid+'" class="userinfo">'+houseZu[i].username+'</a>';
				}else{
					username = houseZu[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+houseZu[i].contact+'</td>');
				var state = "";
				switch (houseZu[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+houseZu[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+houseZu[i].id+'" data-title="'+houseZu[i].title+'" href="houseZuAdd.php?dopost=edit&id='+houseZu[i].id+'" title="修改" class="edit">修改</a>');
