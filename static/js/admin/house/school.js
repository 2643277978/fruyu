		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("school.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("school.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("school.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, school = val.school;
		...
		if(val.state == "100"){
			...
			for(i; i < school.length; i++){
				list.push('<tr data-id="'+school[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+school[i].litpic+'&type=small" class="litpic" />';
				list.push('  <td class="row25 left">'+img+'<span><a href="'+school[i].url+'" target="_blank">'+school[i].title+'</a></span></td>');
				list.push('  <td class="row10">'+school[i].weight+'</td>');

				list.push('  <td class="row12 left">'+school[i].addrname+'<br><small>'+school[i].address+'</small></td>');

				var state = "";
				switch (school[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+school[i].date+'</td>');
				list.push('  <td class="row8">');
				list.push('<a data-id="'+school[i].id+'" data-title="'+school[i].title+'" href="schoolAdd.php?dopost=edit&id='+school[i].id+'" title="修改" class="edit">修改</a>');
