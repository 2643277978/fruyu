$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCf.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCf.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("houseCfEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseCf.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseCf = val.houseCf;
		...
		if(val.state == "100"){
			...
			for(i; i < houseCf.length; i++){
				list.push('<tr data-id="'+houseCf[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+houseCf[i].litpic+'&type=small" class="litpic" />';
				var append = "";
				if(houseCf[i].type == 0){
					append = "(租)";
				}else if(houseCf[i].type == 1){
					append = "(转)";
				}else if(houseCf[i].type == 2){
					append = "(售)";
				}
				list.push('  <td class="row30 left">'+img+'<span><a href="'+houseCf[i].url+'" target="_blank">'+houseCf[i].title+'</a><span class="append">'+append+'</span></span></td>');
				list.push('  <td class="row5">'+houseCf[i].weight+'</td>');
                list.push('  <td class="row9">'+houseCf[i].addrname+'</td>');
				var type = houseCf[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseCf[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseCf[i].userid+'">'+houseCf[i].username+'</a>';
				}else{
					username = houseCf[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+houseCf[i].contact+'</td>');
				var state = "";
				switch (houseCf[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+houseCf[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+houseCf[i].id+'" data-title="'+houseCf[i].title+'" href="houseCfAdd.php?dopost=edit&id='+houseCf[i].id+'" title="修改" class="edit">修改</a>');
