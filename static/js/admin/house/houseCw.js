$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCw.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCw.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("houseCwEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("houseCw.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, houseCw = val.houseCw;
		...
		if(val.state == "100"){
			...
			for(i; i < houseCw.length; i++){
				list.push('<tr data-id="'+houseCw[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+houseCw[i].litpic+'&type=small" class="litpic" />';
				var append = houseCw[i].type == 0 ? "(租)" : (houseCw[i].type == 1 ? "(售)" : "(转)");
				list.push('  <td class="row30 left">'+img+'<span><a href="'+houseCw[i].url+'" target="_blank">'+houseCw[i].title+'</a><span class="append">'+append+'</span></span></td>');
				list.push('  <td class="row5">'+houseCw[i].weight+'</td>');
                list.push('  <td class="row9">'+houseCw[i].addrname+'</td>');
				var type = houseCw[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(houseCw[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+houseCw[i].userid+'" class="userinfo">'+houseCw[i].username+'</a>';
				}else{
					username = houseCw[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+houseCw[i].contact+'</td>');
				var state = "";
				switch (houseCw[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+houseCw[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+houseCw[i].id+'" data-title="'+houseCw[i].title+'" href="houseCwAdd.php?dopost=edit&id='+houseCw[i].id+'" title="修改" class="edit">修改</a>');
